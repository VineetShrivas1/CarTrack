...
namespace CarTrack.DAL
{
    public class CarUtility
    {
        public static ServiceResponseBase AddCar(MyCar car)
        {
            ...
            {
                var user = myContext.UserInformations.FirstOrDefault(x => x.UserID == car.UserID);
                if (user == null)
                    errorCode = "Err0008";
                else
                {
                    ...
                    var oldCar = myContext.MyCars.FirstOrDefault(x => (x.BeaconID.ToLower() == car.BeaconID.ToLower()
                    || x.BeaconUDID.ToLower() == car.BeaconUDID.ToLower()) && x.IsDeleted == false);

                    var deletedCar = myContext.MyCars.FirstOrDefault(x => (x.BeaconID.ToLower() == car.BeaconID.ToLower()
                    || x.BeaconUDID.ToLower() == car.BeaconUDID.ToLower()) && x.IsDeleted == true);
                    ...
                    else if (deletedCar != null)
                    {
                        ...
                        else if (defaultSetting != null && defaultSetting.AddDeviceAfterDays.HasValue
